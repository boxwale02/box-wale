...
export default function QuoteDrawer({ isOpen, onClose, product }: QuoteDrawerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ============================================================
      // ✅ STEP 2: SAVE TO GOOGLE SHEET - CHANGES YAHAN SE
      // ============================================================
      console.log("📤 Saving to Google Sheet...");
      console.log("Payload Sending To Google");
      ...
      const googleResponse = await fetch("/api/excel-script", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      console.log("GOOGLE_SCRIPT_URL =", process.env.NEXT_PUBLIC_GOOGLE_SCRIPT_URL);
      const googleData = await googleResponse.json();

      if (!googleResponse.ok || !googleData.success) {
        throw new Error(googleData.error || "Failed to save to Google Sheet");
      }
...
"
              ...
              {isSubmitted && (
                ...
                >
                  ...
                  <p className="mt-2 text-gray-600">Your quote request has been submitted successfully. We'll get back to you within 24 hours.</p>
                </motion.div>
              )}

              {/* Quote Form - Hide when submitted */}
              {!isSubmitted && (
                <>
                  <div>
                    <div className="flex items-center gap-2 mb-4">
...
"
                      ...
                      <div>
                        ...
                        <PhoneInput
                          ...
                          disabled={isSubmitting}
                          inputClassName="!w-full !h-[46px] !rounded-xl !border !border-gray-300 focus:!border-amber-500"
                          countrySelectorStyleProps={{
                            buttonClassName: "!border-gray-300 !rounded-l-xl",
                          }}
                        />
                      </div>
